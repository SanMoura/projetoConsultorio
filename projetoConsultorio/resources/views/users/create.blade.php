                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ __('Novo Usuário') }}</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ route('user.index') }}" class="btn btn-sm btn-default">{{ __('Voltar para lista') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('user.store') }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group">
                                    <label class="form-control-label" for="input-tipo-usuario">{{ __('Tipo de Usuário') }}</label>
                                    <select name="tipo_usuario_id" id="input-tipo-usuario" class="form-control form-control-alternative">
                                            <option value="0">Escolha...</option>
                                        @forelse ($tipoUsuarios as $tipoUsuario)
                                            <option value="{{$tipoUsuario->id}}">{{$tipoUsuario->ds_tipo_usuario}}</option>
                                        @empty
